...
export default {
  ...
  methods: {
    ...
    // getAppId() {
    //   // 獲取appId
    //   fetch(`/hjBaseUrl/order/api/h5/activityOrder/getAppId`)
    //     .then(res => res.json())
    //     .then(data => {
    ...
    conmmonLogin(scope) {
      ...
      this.$fetch(
        `/hjBaseUrl/stu/api/h5/weixin/loginByCode?code=${code}&scope=${scope}&openId=${openId}`
      )
        // .then(res => res.json())
        .then(data => {
          this.isRequest = true;
          let status = data.result.resultCode;
          this.unionId = data.result.unionId;
          this.wxNickname = data.result.wxNickname;
          this.modulus = data.result.modulus;
          ...
          switch (status) {
            ...
            case 1: //换没有注册
              ...
              this.Dialog = true; //没有注册的时候 弹出注册框
              break;
          }
        })
        .catch(err => alert(err));
      ...
      if (!this.isShow && this.isRequest) {
        ...
        this.$post(
          ...
          .then(response => {
            ...
            //这里调起微信支付
            //   console.log(response.result.wxPayPayResult);
    ...
    goLogin(val) {
      this.$fetch(
        `/hjBaseUrl/stu/api/h5/weixin/refreshModulus?openId=${localStorage.getItem(
          "openId"
        )}`
      )
        //    .then(res=>res.json())
        .then(res => {
          ...
          if (val[0] && val[1]) {
            return this.$post(`/authBaseUrl/api/privDomainBindMobile`, data, {
